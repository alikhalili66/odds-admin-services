...
public class Biz_02_GroupUpdate {
    ...
    public static void update(SQLConnection sqlConnection, JsonObject message, Handler<AsyncResult<JsonObject>> resultHandler) {
        ...
        DAO_Group.fetchById(sqlConnection, message.getInteger("groupId")).onComplete(handler0 -> {
            ...
            Future<Void> futUpdateGroup = DAO_Group.update(sqlConnection, message);
			Future<Void> futSaveGroupHistory = DAO_Group.saveHistory(sqlConnection, joGroup, HistoryEnum.UPDATE.getSymbol(), " ", message.getInteger("userId"));
            
            CompositeFuture.all(futUpdateGroup, futSaveGroupHistory).onComplete(handler -> {
            	if (handler.failed()) {
