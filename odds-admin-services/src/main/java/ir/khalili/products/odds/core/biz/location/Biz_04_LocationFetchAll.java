...
public class Biz_04_LocationFetchAll {

    private static final Logger logger = LogManager.getLogger(Biz_04_LocationFetchAll.class);
    ...
    public static void fetchAll(Vertx vertx, SQLConnection sqlConnection, JsonObject message, Handler<AsyncResult<JsonObject>> resultHandler) {
    	...
    	DAO_Location.fetchAll(sqlConnection, message).onComplete(result -> {
            if (result.failed()) {
