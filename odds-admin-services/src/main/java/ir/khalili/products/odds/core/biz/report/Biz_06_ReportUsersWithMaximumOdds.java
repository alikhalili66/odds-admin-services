...
public class Biz_06_ReportUsersWithMaximumOdds {

    private static final Logger logger = LogManager.getLogger(Biz_06_ReportUsersWithMaximumOdds.class);

    public static void fetchReportUsersWithMaximumOdds(SQLConnection sqlConnection, JsonObject message, Handler<AsyncResult<JsonObject>> resultHandler) {
        ...
        if (isLive != null && isLive.booleanValue()) {
        	...
        	DAO_Report.fetchUsersWithMaximumOdds(sqlConnection).onComplete(result -> {
        		if (result.failed()) {
        		...
        		joReport.put("type", ReportEnum.USERS_WITH_MAXIMUM_ODDS.name());
        		...
        		DAO_Report.saveReport(sqlConnection, joReport).onComplete(result0 -> {
        			...
        			fetchReport(sqlConnection, null, null, ReportEnum.USERS_WITH_MAXIMUM_ODDS.name(), resultHandler);
        		});
