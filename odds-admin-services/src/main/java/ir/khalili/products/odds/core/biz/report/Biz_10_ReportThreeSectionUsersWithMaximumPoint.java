...
public class Biz_10_ReportThreeSectionUsersWithMaximumPoint {

    private static final Logger logger = LogManager.getLogger(Biz_10_ReportThreeSectionUsersWithMaximumPoint.class);

    public static void fetchReportThreeSectionUsersWithMaximumPoint(SQLConnection sqlConnection, JsonObject message, Handler<AsyncResult<JsonObject>> resultHandler) {
    ...
    private static void fetchReport(SQLConnection sqlConnection, Integer competitionId, Integer leagueId, Integer groupId, Integer questionId, Handler<AsyncResult<JsonObject>> resultHandler) {
    	...
    	DAO_Report.fetchReport(sqlConnection, competitionId, leagueId, groupId, questionId, ReportEnum.REPORT_THREE_SECTION_USERS_WITH_MAXIMUM_POINT.name(), false).onComplete(result0 -> {
            if (result0.failed()) {
            	logger.error("Unable to complete result0: " + result0.cause());
                resultHandler.handle(Future.failedFuture(result0.cause()));
                return;
