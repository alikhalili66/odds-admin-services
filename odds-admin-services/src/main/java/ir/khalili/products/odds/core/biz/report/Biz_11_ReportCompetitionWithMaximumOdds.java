...
public class Biz_11_ReportCompetitionWithMaximumOdds {

    private static final Logger logger = LogManager.getLogger(Biz_11_ReportCompetitionWithMaximumOdds.class);

    public static void fetchReportCompetitionWithMaximumOdds(SQLConnection sqlConnection, JsonObject message, Handler<AsyncResult<JsonObject>> resultHandler) {
        ...
        if (isLive != null && isLive.booleanValue()) {
        	...
        	DAO_Report.fetchReportCompetitionWithMaximumOdds(sqlConnection).onComplete(result -> {
        		if (result.failed()) {
        		...
        		joReport.put("type", ReportEnum.COMPETITION_WITH_MAXIMUM_ODDS.name());
        		...
        		DAO_Report.saveReport(sqlConnection, joReport).onComplete(result0 -> {
        			...
        			fetchReport(sqlConnection, null, null, ReportEnum.COMPETITION_WITH_MAXIMUM_ODDS.name(), resultHandler);
        		});
    ...
    private static void fetchReport(SQLConnection sqlConnection, Integer competitionId, Integer leagueId, String type, Handler<AsyncResult<JsonObject>> resultHandler) {
    	...
    	DAO_Report.fetchReport(sqlConnection, competitionId, leagueId, type).onComplete(result0 -> {
            ...
            logger.trace("FETCH_REPORT_COMPETITION_WITH_MAXIMUM_ODDS_RESULT : " + result0.result());
