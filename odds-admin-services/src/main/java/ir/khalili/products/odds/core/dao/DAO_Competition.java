...
public class DAO_Competition {
    ...
    public static Future<JsonObject> fetchCompetitionById(SQLConnection sqlConnection, int competitionId) {
        Promise<JsonObject> promise = Promise.promise();
        JsonArray params = new JsonArray();
        params.add(competitionId);

        sqlConnection.queryWithParams("SELECT * FROM TOPPCOMPETITION TC WHERE TC.ID = ? AND TC.DTO IS NULL ",params ,resultHandler -> {
            if (resultHandler.failed()) {
                logger.error("Unable to get accessQueryResult:", resultHandler.cause());
                promise.fail(new DAOEXCP_Internal(-100, "خطای داخلی. با راهبر سامانه تماس بگیرید."));
            } else {
    ...
    public static Future<Void> saveQuestionCorrectAnswer(SQLConnection sqlConnection, JsonObject jsonObject, Map<String, String> liveScoreMap) {
        ...
        for (Iterator<String> iterator = liveScoreMap.keySet().iterator(); iterator.hasNext(); ) {
            ...
            params.add(new JsonArray().add(liveScoreMap.get(symbol)).add(jsonObject.getInteger("competitionId")).add(symbol).add(jsonObject.getInteger("leadgueId")));
        }

        sqlConnection.batchWithParams("UPDATE toppcompetitionquestion " +
                " SET result = ? " +
                " WHERE competition_id = ? " +
                " AND question_id = (SELECT q.id FROM toppquestion q WHERE q.symbol = ? and q.league_id = ? AND ROWNUM = 1)", params, resultHandler -> {
            if (resultHandler.failed()) {
                logger.error("Unable to get accessQueryResult:", resultHandler.cause());
                promise.fail(new DAOEXCP_Internal(-100, "خطای داخلی. با راهبر سامانه تماس بگیرید."));
                return;
