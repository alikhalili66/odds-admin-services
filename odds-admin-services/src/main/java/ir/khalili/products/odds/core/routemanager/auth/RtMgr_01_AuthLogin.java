package ir.khalili.products.odds.core.routemanager.auth;
 ...
 * @author A.KH
 */
public class RtMgr_01_AuthLogin   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_01_AuthLogin.class);
	
	public static void handler(RoutingContext context) {
		
		AuthInputValidationUtil.validateLogin(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_AUTH_LOGIN, validateHandler.result(),replyHandler->{
