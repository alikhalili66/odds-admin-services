 ...
 * @author A.KH
 */
public class RtMgr_02_AuthOTP   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_02_AuthOTP.class);
	
	public static void handler(RoutingContext context) {
		
		AuthInputValidationUtil.validateOTP(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_AUTH_OTP, validateHandler.result(),replyHandler->{
