 ...
 * @author A.KH
 */
public class RtMgr_01_CompetitionSave   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_01_CompetitionSave.class);
	
	public static void handler(RoutingContext context) {
		
		CompetitionInputValidationUtil.validateSave(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_COMPETITION_SAVE, validateHandler.result(),replyHandler->{
