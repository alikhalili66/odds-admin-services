 ...
 * @author A.KH
 */
public class RtMgr_04_CompetitionFetchAll   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_04_CompetitionFetchAll.class);
	
	public static void handler(RoutingContext context) {
		
		CompetitionInputValidationUtil.validateFetchAll(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_COMPETITION_ALL_FETCH, validateHandler.result(),replyHandler->{
