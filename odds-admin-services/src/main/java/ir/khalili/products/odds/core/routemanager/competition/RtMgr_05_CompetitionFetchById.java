 ...
 * @author A.KH
 */
public class RtMgr_05_CompetitionFetchById   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_05_CompetitionFetchById.class);
	
	public static void handler(RoutingContext context) {
		
		CompetitionInputValidationUtil.validateFetchById(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_COMPETITION_ID_FETCH, validateHandler.result(),replyHandler->{
