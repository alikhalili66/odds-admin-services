 ...
 * @author A.KH
 */
public class RtMgr_08_CompetitionQuestionUnAssign   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_08_CompetitionQuestionUnAssign.class);
	
	public static void handler(RoutingContext context) {
		
		CompetitionInputValidationUtil.validateQuestionUnAssign(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_COMPETITION_QUESTION_UNASSIGN, validateHandler.result(),replyHandler->{
