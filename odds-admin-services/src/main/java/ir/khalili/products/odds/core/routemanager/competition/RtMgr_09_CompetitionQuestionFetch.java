 ...
 * @author A.KH
 */
public class RtMgr_09_CompetitionQuestionFetch   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_09_CompetitionQuestionFetch.class);
	
	public static void handler(RoutingContext context) {
		
		CompetitionInputValidationUtil.validateQuestionFetch(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_COMPETITION_QUESTION_FETCH, validateHandler.result(),replyHandler->{
