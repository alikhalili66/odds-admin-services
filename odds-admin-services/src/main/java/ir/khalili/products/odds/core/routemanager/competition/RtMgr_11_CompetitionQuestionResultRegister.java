 ...
 * @author A.KH
 */
public class RtMgr_11_CompetitionQuestionResultRegister   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_11_CompetitionQuestionResultRegister.class);
	
	public static void handler(RoutingContext context) {
		
		CompetitionInputValidationUtil.validateQuestionResultRegister(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_COMPETITION_QUESTION_RESULT_REGISTER, validateHandler.result(),replyHandler->{
