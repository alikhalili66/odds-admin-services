 ...
 * @author A.KH
 */
public class RtMgr_12_CompetitionPointCalculation   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_12_CompetitionPointCalculation.class);
	
	public static void handler(RoutingContext context) {
		
		CompetitionInputValidationUtil.validatePointCalculation(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_COMPETITION_POINT_CALCULATION, validateHandler.result(),replyHandler->{
