 ...
 * @author A.KH
 */
public class RtMgr_13_CompetitionLiveScore   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_13_CompetitionLiveScore.class);
	
	public static void handler(RoutingContext context) {
		
		CompetitionInputValidationUtil.validateLiveScore(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_COMPETITION_LIVE_SCORE, validateHandler.result(),replyHandler->{
