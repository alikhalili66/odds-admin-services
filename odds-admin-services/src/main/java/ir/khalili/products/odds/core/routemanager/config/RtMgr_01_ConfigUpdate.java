 ...
 * @author A.KH
 */
public class RtMgr_01_ConfigUpdate   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_01_ConfigUpdate.class);
	
	public static void handler(RoutingContext context) {
		
		ConfigInputValidationUtil.validateUpdate(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_CONFIG_UPDATE, validateHandler.result(),replyHandler->{
