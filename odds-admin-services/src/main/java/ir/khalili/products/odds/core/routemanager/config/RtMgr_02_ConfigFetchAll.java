 ...
 * @author A.KH
 */
public class RtMgr_02_ConfigFetchAll   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_02_ConfigFetchAll.class);
	
	public static void handler(RoutingContext context) {
		
		ConfigInputValidationUtil.validateFetchAll(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_CONFIG_ALL_FETCH, validateHandler.result(),replyHandler->{
