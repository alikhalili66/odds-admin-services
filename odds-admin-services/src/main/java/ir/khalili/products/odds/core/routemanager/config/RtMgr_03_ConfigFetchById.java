 ...
 * @author A.KH
 */
public class RtMgr_03_ConfigFetchById   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_03_ConfigFetchById.class);
	
	public static void handler(RoutingContext context) {
		
		ConfigInputValidationUtil.validateFetchById(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_CONFIG_ID_FETCH, validateHandler.result(),replyHandler->{
