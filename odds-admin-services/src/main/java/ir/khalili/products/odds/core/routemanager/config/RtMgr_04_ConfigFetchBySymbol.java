package ir.khalili.products.odds.core.routemanager.config;
...
import ir.khalili.products.odds.core.validation.ConfigInputValidationUtil;

/**
 * @author A.KH
 */
public class RtMgr_04_ConfigFetchBySymbol   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_04_ConfigFetchBySymbol.class);
	
	public static void handler(RoutingContext context) {
		
		ConfigInputValidationUtil.validateFetchBySymbol(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_CONFIG_SYMBOL_FETCH, validateHandler.result(),replyHandler->{
