 ...
 * @author A.KH
 */
public class RtMgr_01_FolderSave   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_01_FolderSave.class);
	
	public static void handler(RoutingContext context) {
		
		FolderInputValidationUtil.validateSave(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_SAVE, validateHandler.result(),replyHandler->{
