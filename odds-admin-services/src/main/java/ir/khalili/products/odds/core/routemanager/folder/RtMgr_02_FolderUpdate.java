 ...
 * @author A.KH
 */
public class RtMgr_02_FolderUpdate   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_02_FolderUpdate.class);
	
	public static void handler(RoutingContext context) {
		
		FolderInputValidationUtil.validateUpdate(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_UPDATE, validateHandler.result(),replyHandler->{
