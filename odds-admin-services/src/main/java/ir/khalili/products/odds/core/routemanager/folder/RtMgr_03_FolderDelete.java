 ...
 * @author A.KH
 */
public class RtMgr_03_FolderDelete   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_03_FolderDelete.class);
	
	public static void handler(RoutingContext context) {
		
		FolderInputValidationUtil.validateDelete(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_DELETE, validateHandler.result(),replyHandler->{
