 ...
 * @author A.KH
 */
public class RtMgr_04_FolderFetchAll   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_04_FolderFetchAll.class);
	
	public static void handler(RoutingContext context) {
		
		FolderInputValidationUtil.validateFetchAll(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_ALL_FETCH, validateHandler.result(),replyHandler->{
