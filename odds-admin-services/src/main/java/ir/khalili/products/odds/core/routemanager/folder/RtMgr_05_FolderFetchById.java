 ...
 * @author A.KH
 */
public class RtMgr_05_FolderFetchById   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_05_FolderFetchById.class);
	
	public static void handler(RoutingContext context) {
		
		FolderInputValidationUtil.validateFetchById(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_ID_FETCH, validateHandler.result(),replyHandler->{
