 ...
 * @author A.KH
 */
public class RtMgr_06_FolderQuestionAssign   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_06_FolderQuestionAssign.class);
	
	public static void handler(RoutingContext context) {
		
		FolderInputValidationUtil.validateQuestionAssign(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_QUESTION_ASSIGN, validateHandler.result(),replyHandler->{
