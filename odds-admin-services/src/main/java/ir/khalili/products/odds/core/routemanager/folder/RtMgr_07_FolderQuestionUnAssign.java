package ir.khalili.products.odds.core.routemanager.folder;
...
import ir.khalili.products.odds.core.validation.FolderInputValidationUtil;

/**
 * @author A.KH
 */
public class RtMgr_07_FolderQuestionUnAssign   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_07_FolderQuestionUnAssign.class);
	
	public static void handler(RoutingContext context) {
		
		FolderInputValidationUtil.validateQuestionUnAssign(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_QUESTION_UNASSIGN, validateHandler.result(),replyHandler->{
