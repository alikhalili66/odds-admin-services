 ...
 * @author A.KH
 */
public class RtMgr_08_FolderQuestionFetch   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_08_FolderQuestionFetch.class);
	
	public static void handler(RoutingContext context) {
		
		FolderInputValidationUtil.validateQuestionFetch(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_QUESTION_FETCH, validateHandler.result(),replyHandler->{
