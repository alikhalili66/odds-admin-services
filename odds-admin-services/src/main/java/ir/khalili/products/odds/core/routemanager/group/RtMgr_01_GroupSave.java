 ...
 * @author A.KH
 */
public class RtMgr_01_GroupSave   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_01_GroupSave.class);
	
	public static void handler(RoutingContext context) {
		
		GroupInputValidationUtil.validateSave(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_GROUP_SAVE, validateHandler.result(),replyHandler->{
