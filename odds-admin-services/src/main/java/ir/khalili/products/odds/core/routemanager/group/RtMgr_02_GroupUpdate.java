 ...
 * @author A.KH
 */
public class RtMgr_02_GroupUpdate   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_02_GroupUpdate.class);
	
	public static void handler(RoutingContext context) {
		
		GroupInputValidationUtil.validateUpdate(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_GROUP_UPDATE, validateHandler.result(),replyHandler->{
