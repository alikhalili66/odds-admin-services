 ...
 * @author A.KH
 */
public class RtMgr_03_GroupDelete   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_03_GroupDelete.class);
	
	public static void handler(RoutingContext context) {
		
		GroupInputValidationUtil.validateDelete(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_GROUP_DELETE, validateHandler.result(),replyHandler->{
