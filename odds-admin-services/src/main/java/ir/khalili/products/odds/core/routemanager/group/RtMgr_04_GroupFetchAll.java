 ...
 * @author A.KH
 */
public class RtMgr_04_GroupFetchAll   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_04_GroupFetchAll.class);
	
	public static void handler(RoutingContext context) {
		
		GroupInputValidationUtil.validateFetchAll(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_GROUP_ALL_FETCH, validateHandler.result(),replyHandler->{
