 ...
 * @author A.KH
 */
public class RtMgr_05_GroupFetchById   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_05_GroupFetchById.class);
	
	public static void handler(RoutingContext context) {
		
		GroupInputValidationUtil.validateFetchById(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_GROUP_ID_FETCH, validateHandler.result(),replyHandler->{
