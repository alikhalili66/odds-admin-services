 ...
 * @author A.KH
 */
public class RtMgr_07_GroupTeamUnAssign   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_07_GroupTeamUnAssign.class);
	
	public static void handler(RoutingContext context) {
		
		GroupInputValidationUtil.validateTeamUnAssign(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_GROUP_TEAM_UNASSIGN, validateHandler.result(),replyHandler->{
