 ...
 * @author A.KH
 */
public class RtMgr_01_LeagueSave   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_01_LeagueSave.class);
	
	public static void handler(RoutingContext context) {
		
		LeagueInputValidationUtil.validateSave(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_LEAGUE_SAVE, validateHandler.result(),replyHandler->{
