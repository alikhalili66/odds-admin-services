 ...
 * @author A.KH
 */
public class RtMgr_03_LeagueDelete   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_03_LeagueDelete.class);
	
	public static void handler(RoutingContext context) {
		
		LeagueInputValidationUtil.validateDelete(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_LEAGUE_DELETE, validateHandler.result(),replyHandler->{
