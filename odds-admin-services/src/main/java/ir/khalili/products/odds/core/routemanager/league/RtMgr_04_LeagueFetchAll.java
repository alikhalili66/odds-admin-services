 ...
 * @author A.KH
 */
public class RtMgr_04_LeagueFetchAll   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_04_LeagueFetchAll.class);
	
	public static void handler(RoutingContext context) {
		
		LeagueInputValidationUtil.validateFetchAll(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_LEAGUE_ALL_FETCH, validateHandler.result(),replyHandler->{
