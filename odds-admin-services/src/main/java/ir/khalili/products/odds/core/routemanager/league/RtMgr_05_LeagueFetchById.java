package ir.khalili.products.odds.core.routemanager.league;
...
import ir.khalili.products.odds.core.validation.LeagueInputValidationUtil;

/**
 * @author A.KH
 */
public class RtMgr_05_LeagueFetchById   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_05_LeagueFetchById.class);
	
	public static void handler(RoutingContext context) {
		
		LeagueInputValidationUtil.validateFetchById(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_LEAGUE_ID_FETCH, validateHandler.result(),replyHandler->{
