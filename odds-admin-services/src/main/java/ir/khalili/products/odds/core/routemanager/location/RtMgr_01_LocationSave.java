 ...
 * @author A.KH
 */
public class RtMgr_01_LocationSave   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_01_LocationSave.class);
	
	public static void handler(RoutingContext context) {
		
		LocationInputValidationUtil.validateSave(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_LOCATION_SAVE, validateHandler.result(),replyHandler->{
