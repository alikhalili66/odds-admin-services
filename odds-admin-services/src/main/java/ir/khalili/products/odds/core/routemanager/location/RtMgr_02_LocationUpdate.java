 ...
 * @author A.KH
 */
public class RtMgr_02_LocationUpdate   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_02_LocationUpdate.class);
	
	public static void handler(RoutingContext context) {
		
		LocationInputValidationUtil.validateUpdate(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_LOCATION_UPDATE, validateHandler.result(),replyHandler->{
