 ...
 * @author A.KH
 */
public class RtMgr_03_LocationDelete   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_03_LocationDelete.class);
	
	public static void handler(RoutingContext context) {
		
		LocationInputValidationUtil.validateDelete(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_LOCATION_DELETE, validateHandler.result(),replyHandler->{
