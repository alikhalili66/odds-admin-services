 ...
 * @author A.KH
 */
public class RtMgr_04_LocationFetchAll   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_04_LocationFetchAll.class);
	
	public static void handler(RoutingContext context) {
		
		LocationInputValidationUtil.validateFetchAll(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_LOCATION_ALL_FETCH, validateHandler.result(),replyHandler->{
