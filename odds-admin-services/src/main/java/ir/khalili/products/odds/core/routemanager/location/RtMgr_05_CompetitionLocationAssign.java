package ir.khalili.products.odds.core.routemanager.location;
...
import ir.khalili.products.odds.core.validation.LocationInputValidationUtil;

/**
 * @author A.KH
 */
public class RtMgr_05_CompetitionLocationAssign   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_05_CompetitionLocationAssign.class);
	
	public static void handler(RoutingContext context) {
		
		LocationInputValidationUtil.validateLocationAssign(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_COMPETITION_LOCATION_ASSIGN, validateHandler.result(),replyHandler->{
