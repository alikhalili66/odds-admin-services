 ...
 * @author A.KH
 */
public class RtMgr_02_QuestionUpdate   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_02_QuestionUpdate.class);
	...
	public static void handler(RoutingContext context) {
		...
		AuthInputValidationUtil.validateAgent(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_QUESTION_UPDATE, validateHandler.result(),replyHandler->{
