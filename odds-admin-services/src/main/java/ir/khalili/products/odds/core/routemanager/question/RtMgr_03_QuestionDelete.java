 ...
 * @author A.KH
 */
public class RtMgr_03_QuestionDelete   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_03_QuestionDelete.class);
	
	public static void handler(RoutingContext context) {
		
		QuestionInputValidationUtil.validateDelete(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_QUESTION_DELETE, validateHandler.result(),replyHandler->{
