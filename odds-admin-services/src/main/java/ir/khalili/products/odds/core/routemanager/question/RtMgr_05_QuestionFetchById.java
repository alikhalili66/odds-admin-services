 ...
 * @author A.KH
 */
public class RtMgr_05_QuestionFetchById   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_05_QuestionFetchById.class);
	
	public static void handler(RoutingContext context) {
		
		QuestionInputValidationUtil.validateFetchById(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_QUESTION_ID_FETCH, validateHandler.result(),replyHandler->{
