 ...
 * @author A.KH
 */
public class RtMgr_01_ReportRegisteredUsersCount   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_01_ReportRegisteredUsersCount.class);
	
	public static void handler(RoutingContext context) {
		
		ReportInputValidationUtil.validateRegisteredUsersCount(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_REPORT_REGISTERED_USERS_COUNT, validateHandler.result(),replyHandler->{
