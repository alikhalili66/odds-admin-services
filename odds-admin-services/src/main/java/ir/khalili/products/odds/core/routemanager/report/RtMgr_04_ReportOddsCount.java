 ...
 * @author A.KH
 */
public class RtMgr_04_ReportOddsCount   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_04_ReportOddsCount.class);
	
	public static void handler(RoutingContext context) {
		
		ReportInputValidationUtil.validateOddsCount(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_REPORT_ODDS_COUNT, validateHandler.result(),replyHandler->{
