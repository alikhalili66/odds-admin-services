 ...
 * @author A.KH
 */
public class RtMgr_07_ReportOddsCountPerCompetition   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_07_ReportOddsCountPerCompetition.class);
	
	public static void handler(RoutingContext context) {
		
		ReportInputValidationUtil.validateOddsCountPerCompetition(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_REPORT_ODDS_COUNT_PER_COMPETITION, validateHandler.result(),replyHandler->{
