 ...
 * @author A.KH
 */
public class RtMgr_10_ReportThreeSectionUsersWithMaximumPoint   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_10_ReportThreeSectionUsersWithMaximumPoint.class);
	
	public static void handler(RoutingContext context) {
		
		ReportInputValidationUtil.validateThreeSectionUsersWithMaximumPoint(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_REPORT_THREE_SECTION_USERS_WITH_MAXIMUM_POINT, validateHandler.result(),replyHandler->{
