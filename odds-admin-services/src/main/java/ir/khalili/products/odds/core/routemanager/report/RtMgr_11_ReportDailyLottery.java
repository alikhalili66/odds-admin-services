 ...
 * @author A.KH
 */
public class RtMgr_11_ReportDailyLottery   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_11_ReportDailyLottery.class);
	
	public static void handler(RoutingContext context) {
		
		ReportInputValidationUtil.validateReportDialyLottery(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_REPORT_DAILY_LOTTERY, validateHandler.result(), deliveryOptions, replyHandler->{
