 ...
 * @author A.KH
 */
public class RtMgr_11_ReportLeagueTransactionAmount   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_11_ReportLeagueTransactionAmount.class);
	
	public static void handler(RoutingContext context) {
		
		ReportInputValidationUtil.validateLeagueTransactionAmount(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_REPORT_LEAGUE_TRANSACTION_AMOUNT, validateHandler.result(),replyHandler->{
