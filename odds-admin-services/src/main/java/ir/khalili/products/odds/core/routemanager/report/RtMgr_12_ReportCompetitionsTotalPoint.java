 ...
 * @author A.KH
 */
public class RtMgr_12_ReportCompetitionsTotalPoint   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_12_ReportCompetitionsTotalPoint.class);
	
	public static void handler(RoutingContext context) {
		
		ReportInputValidationUtil.validateCompetitionsTotalPoint(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_REPORT_COMPETITIONS_TOTAL_POINT, validateHandler.result(),replyHandler->{
