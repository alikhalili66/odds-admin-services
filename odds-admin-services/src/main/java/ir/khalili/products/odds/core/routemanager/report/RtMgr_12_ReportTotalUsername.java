 ...
 * @author A.KH
 */
public class RtMgr_12_ReportTotalUsername   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_12_ReportTotalUsername.class);
	
	public static void handler(RoutingContext context) {
		
		ReportInputValidationUtil.validateReportTotalUsername(context, validateHandler->{
			...
			DeliveryOptions deliveryOptions = new DeliveryOptions().setSendTimeout(EntryPoint.MAX_TIMEOUT);
			
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_REPORT_TOTAL_USERNAME, validateHandler.result(), deliveryOptions, replyHandler->{
