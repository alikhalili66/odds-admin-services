 ...
 * @author A.KH
 */
public class RtMgr_13_ReportDailyOdds   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_13_ReportDailyOdds.class);
	
	public static void handler(RoutingContext context) {
		
		ReportInputValidationUtil.validateReportDialyOdds(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_REPORT_DAILY_ODDS, validateHandler.result(), deliveryOptions, replyHandler->{
