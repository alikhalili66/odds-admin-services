 ...
 * @author A.KH
 */
public class RtMgr_01_TeamSave   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_01_TeamSave.class);
	
	public static void handler(RoutingContext context) {
		
		TeamInputValidationUtil.validateSave(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TEAM_SAVE, validateHandler.result(),replyHandler->{
