 ...
 * @author A.KH
 */
public class RtMgr_02_TeamUpdate   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_02_TeamUpdate.class);
	
	public static void handler(RoutingContext context) {
		
		TeamInputValidationUtil.validateUpdate(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TEAM_UPDATE, validateHandler.result(),replyHandler->{
