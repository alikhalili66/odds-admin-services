 ...
 * @author A.KH
 */
public class RtMgr_03_TeamDelete   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_03_TeamDelete.class);
	
	public static void handler(RoutingContext context) {
		
		TeamInputValidationUtil.validateDelete(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TEAM_DELETE, validateHandler.result(),replyHandler->{
