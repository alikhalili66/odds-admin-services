 ...
 * @author A.KH
 */
public class RtMgr_04_TeamFetchAll   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_04_TeamFetchAll.class);
	
	public static void handler(RoutingContext context) {
		
		TeamInputValidationUtil.validateFetchAll(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TEAM_ALL_FETCH, validateHandler.result(),replyHandler->{
