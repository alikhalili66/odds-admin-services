 ...
 * @author A.KH
 */
public class RtMgr_05_TeamFetchById   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_05_TeamFetchById.class);
	
	public static void handler(RoutingContext context) {
		
		TeamInputValidationUtil.validateFetchById(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TEAM_ID_FETCH, validateHandler.result(),replyHandler->{
