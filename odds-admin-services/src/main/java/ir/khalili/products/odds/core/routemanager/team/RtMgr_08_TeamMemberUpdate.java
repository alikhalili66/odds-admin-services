 ...
 * @author A.KH
 */
public class RtMgr_08_TeamMemberUpdate   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_08_TeamMemberUpdate.class);
	
	public static void handler(RoutingContext context) {
		
		TeamInputValidationUtil.validateMemberUpdate(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TEAM_MEMBER_UPDATE, validateHandler.result(),replyHandler->{
