package ir.khalili.products.odds.core.routemanager.team;
...
import ir.khalili.products.odds.core.validation.TeamInputValidationUtil;

/**
 * @author A.KH
 */
public class RtMgr_09_TeamMemberDelete   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_09_TeamMemberDelete.class);
	
	public static void handler(RoutingContext context) {
		
		TeamInputValidationUtil.validateMemberDelete(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TEAM_MEMBER_DELETE, validateHandler.result(),replyHandler->{
