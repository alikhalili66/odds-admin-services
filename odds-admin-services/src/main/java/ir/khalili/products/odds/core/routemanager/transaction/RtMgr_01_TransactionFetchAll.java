package ir.khalili.products.odds.core.routemanager.transaction;
...
import ir.khalili.products.odds.core.validation.TransactionInputValidationUtil;

/**
 * @author A.KH
 */
public class RtMgr_01_TransactionFetchAll   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_01_TransactionFetchAll.class);
	
	public static void handler(RoutingContext context) {
		
		TransactionInputValidationUtil.validateTransactionFetchAll(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TRANSACTION_ALL_FETCH, validateHandler.result(),replyHandler->{
