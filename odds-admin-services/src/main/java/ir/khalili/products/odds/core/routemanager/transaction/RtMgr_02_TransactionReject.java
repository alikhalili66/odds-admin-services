 ...
 * @author A.KH
 */
public class RtMgr_02_TransactionReject   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_02_TransactionReject.class);
	
	public static void handler(RoutingContext context) {
		
		TransactionInputValidationUtil.validateTransactionReject(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TRANSACTION_REJECT, validateHandler.result(),replyHandler->{
