 ...
 * @author A.KH
 */
public class RtMgr_03_TransactionConfirm   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_03_TransactionConfirm.class);
	
	public static void handler(RoutingContext context) {
		
		TransactionInputValidationUtil.validateTransactionConfirm(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TRANSACTION_CONFIRM, validateHandler.result(),replyHandler->{
