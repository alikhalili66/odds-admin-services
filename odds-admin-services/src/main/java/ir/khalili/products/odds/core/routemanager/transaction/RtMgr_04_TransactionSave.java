 ...
 * @author A.KH
 */
public class RtMgr_04_TransactionSave   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_04_TransactionSave.class);
	
	public static void handler(RoutingContext context) {
		
		TransactionInputValidationUtil.validateTransactionSave(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TRANSACTION_SAVE, validateHandler.result(),replyHandler->{
