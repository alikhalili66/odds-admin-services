 ...
 * @author A.KH
 */
public class RtMgr_01_UserFetchAll   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_01_UserFetchAll.class);
	
	public static void handler(RoutingContext context) {
		
		UserInputValidationUtil.validateFetchAll(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_USER_ALL_FETCH, validateHandler.result(),replyHandler->{
