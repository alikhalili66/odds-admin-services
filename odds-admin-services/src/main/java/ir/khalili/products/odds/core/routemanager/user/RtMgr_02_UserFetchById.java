 ...
 * @author A.KH
 */
public class RtMgr_02_UserFetchById   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_02_UserFetchById.class);
	
	public static void handler(RoutingContext context) {
		
		UserInputValidationUtil.validateFetchById(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_USER_ID_FETCH, validateHandler.result(),replyHandler->{
