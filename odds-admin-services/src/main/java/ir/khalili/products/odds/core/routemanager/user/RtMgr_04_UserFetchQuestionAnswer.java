package ir.khalili.products.odds.core.routemanager.user;
...
import ir.khalili.products.odds.core.validation.UserInputValidationUtil;

/**
 * @author A.KH
 */
public class RtMgr_04_UserFetchQuestionAnswer   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_04_UserFetchQuestionAnswer.class);
	
	public static void handler(RoutingContext context) {
		
		UserInputValidationUtil.validateUserFetchQuestionAnswer(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_USER_QUESTION_FETCH, validateHandler.result(),replyHandler->{
