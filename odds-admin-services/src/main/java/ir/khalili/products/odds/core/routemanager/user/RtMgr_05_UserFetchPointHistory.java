 ...
 * @author A.KH
 */
public class RtMgr_05_UserFetchPointHistory   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_05_UserFetchPointHistory.class);
	
	public static void handler(RoutingContext context) {
		
		UserInputValidationUtil.validateUserPointHistory(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_USER_HISTORY_FETCH, validateHandler.result(),replyHandler->{
