 ...
 * @author A.KH
 */
public class RtMgr_06_UserEditNikeName   {
	
	private static								Logger 			logger 								= LogManager.getLogger(RtMgr_06_UserEditNikeName.class);
	
	public static void handler(RoutingContext context) {
		
		UserInputValidationUtil.validateUserNikeName(context, validateHandler->{
			...
			context.vertx().eventBus().request(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_USER_EDIT_NIKENAME, validateHandler.result(),replyHandler->{
