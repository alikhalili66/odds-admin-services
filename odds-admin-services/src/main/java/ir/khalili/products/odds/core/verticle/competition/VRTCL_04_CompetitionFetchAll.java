...
import ir.khalili.products.odds.core.biz.competition.Biz_04_CompetitionFetchAll;
import ir.khalili.products.odds.core.constants.AppConstants;
import ir.khalili.products.odds.core.utils.Configuration;

public class VRTCL_04_CompetitionFetchAll extends AbstractVerticle {
	private Logger logger = LogManager.getLogger(VRTCL_04_CompetitionFetchAll.class);
    ...
    public void start(Promise<Void> startPromise) throws Exception {
    	...
    	try {
        	...
        	vertx.eventBus().consumer(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_COMPETITION_ALL_FETCH, message -> {
        		
        		logger.trace("Event "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_COMPETITION_ALL_FETCH+" recieved with message:"+((JsonObject)(message.body())));
        		...
        		ircJDBC.getConnection(connection -> {
					...
					Biz_04_CompetitionFetchAll.fetchAll(vertx, sqlConnection, (JsonObject)(message.body()), resultHandler -> {
        	...
        	logger.info("Event Bus Handler "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_COMPETITION_ALL_FETCH+" ready to reply.");
