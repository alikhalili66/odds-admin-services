...
import ir.khalili.products.odds.core.biz.competition.Biz_13_CompetitionLiveScore;
import ir.khalili.products.odds.core.constants.AppConstants;
import ir.khalili.products.odds.core.utils.Configuration;

public class VRTCL_13_CompetitionLiveScore extends AbstractVerticle {
	private Logger logger = LogManager.getLogger(VRTCL_13_CompetitionLiveScore.class);
    ...
    public void start(Promise<Void> startPromise) throws Exception {
    	...
    	try {
        	...
        	vertx.eventBus().consumer(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_COMPETITION_LIVE_SCORE, message -> {
        		
        		logger.trace("Event "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_COMPETITION_LIVE_SCORE+" recieved with message:"+((JsonObject)(message.body())));
        		...
        		ircJDBC.getConnection(connection -> {
					...
					Biz_13_CompetitionLiveScore.fetch(sqlConnection, (JsonObject)(message.body()), resultHandler -> {
        	...
        	logger.info("Event Bus Handler "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_COMPETITION_LIVE_SCORE+" ready to reply.");
