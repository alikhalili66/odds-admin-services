...
import ir.khalili.products.odds.core.biz.config.Biz_01_ConfigUpdate;
import ir.khalili.products.odds.core.constants.AppConstants;
import ir.khalili.products.odds.core.utils.Configuration;

public class VRTCL_01_ConfigUpdate extends AbstractVerticle {
	private Logger logger = LogManager.getLogger(VRTCL_01_ConfigUpdate.class);
    ...
    public void start(Promise<Void> startPromise) throws Exception {
    	...
    	try {
        	...
        	vertx.eventBus().consumer(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_CONFIG_UPDATE, message -> {
        		
        		logger.trace("Event "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_CONFIG_UPDATE+" recieved with message:"+((JsonObject)(message.body())));
        		...
        		ircJDBC.getConnection(connection -> {
					...
					Biz_01_ConfigUpdate.update(sqlConnection, (JsonObject)(message.body()), resultHandler -> {
        	...
        	logger.info("Event Bus Handler "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_CONFIG_UPDATE+" ready to reply.");
