package ir.khalili.products.odds.core.verticle.config;
...
import ir.khalili.products.odds.core.biz.config.Biz_04_ConfigFetchBySymbol;
import ir.khalili.products.odds.core.constants.AppConstants;
import ir.khalili.products.odds.core.utils.Configuration;

public class VRTCL_04_ConfigFetchBySymbol extends AbstractVerticle {
	private Logger logger = LogManager.getLogger(VRTCL_04_ConfigFetchBySymbol.class);
    ...
    public void start(Promise<Void> startPromise) throws Exception {
    	...
    	try {
        	...
        	vertx.eventBus().consumer(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_CONFIG_SYMBOL_FETCH, message -> {
        		
        		logger.trace("Event "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_CONFIG_SYMBOL_FETCH+" recieved with message:"+((JsonObject)(message.body())));
        		...
        		ircJDBC.getConnection(connection -> {
					...
					Biz_04_ConfigFetchBySymbol.fetchBySymbol(sqlConnection, (JsonObject)(message.body()), resultHandler -> {
        	...
        	logger.info("Event Bus Handler "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_CONFIG_SYMBOL_FETCH+" ready to reply.");
