...
import ir.khalili.products.odds.core.biz.folder.Biz_06_FolderQuestionAssign;
import ir.khalili.products.odds.core.constants.AppConstants;
import ir.khalili.products.odds.core.utils.Configuration;

public class VRTCL_06_FolderQuestionAssign extends AbstractVerticle {
	private Logger logger = LogManager.getLogger(VRTCL_06_FolderQuestionAssign.class);
    ...
    public void start(Promise<Void> startPromise) throws Exception {
    	...
    	try {
        	...
        	vertx.eventBus().consumer(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_QUESTION_ASSIGN, message -> {
        		
        		logger.trace("Event "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_QUESTION_ASSIGN+" recieved with message:"+((JsonObject)(message.body())));
        		...
        		ircJDBC.getConnection(connection -> {
					...
					Biz_06_FolderQuestionAssign.questionAssign(sqlConnection, (JsonObject)(message.body()), resultHandler -> {
        	...
        	logger.info("Event Bus Handler "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_QUESTION_ASSIGN+" ready to reply.");
