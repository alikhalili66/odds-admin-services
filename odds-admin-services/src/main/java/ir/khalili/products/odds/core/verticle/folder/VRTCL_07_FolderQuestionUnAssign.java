...
import ir.khalili.products.odds.core.biz.folder.Biz_07_FolderQuestionUnAssign;
import ir.khalili.products.odds.core.constants.AppConstants;
import ir.khalili.products.odds.core.utils.Configuration;

public class VRTCL_07_FolderQuestionUnAssign extends AbstractVerticle {
	private Logger logger = LogManager.getLogger(VRTCL_07_FolderQuestionUnAssign.class);
    ...
    public void start(Promise<Void> startPromise) throws Exception {
    	...
    	try {
        	...
        	vertx.eventBus().consumer(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_QUESTION_UNASSIGN, message -> {
        		
        		logger.trace("Event "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_QUESTION_UNASSIGN+" recieved with message:"+((JsonObject)(message.body())));
        		...
        		ircJDBC.getConnection(connection -> {
					...
					Biz_07_FolderQuestionUnAssign.questionUnAssign(sqlConnection, (JsonObject)(message.body()), resultHandler -> {
        	...
        	logger.info("Event Bus Handler "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_QUESTION_UNASSIGN+" ready to reply.");
