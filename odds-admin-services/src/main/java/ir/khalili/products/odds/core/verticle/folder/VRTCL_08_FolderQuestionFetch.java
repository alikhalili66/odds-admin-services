...
import ir.khalili.products.odds.core.biz.folder.Biz_08_FolderQuestionFetch;
import ir.khalili.products.odds.core.constants.AppConstants;
import ir.khalili.products.odds.core.utils.Configuration;

public class VRTCL_08_FolderQuestionFetch extends AbstractVerticle {
	private Logger logger = LogManager.getLogger(VRTCL_08_FolderQuestionFetch.class);
    ...
    public void start(Promise<Void> startPromise) throws Exception {
    	...
    	try {
        	...
        	vertx.eventBus().consumer(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_QUESTION_FETCH, message -> {
        		
        		logger.trace("Event "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_QUESTION_FETCH+" recieved with message:"+((JsonObject)(message.body())));
        		...
        		ircJDBC.getConnection(connection -> {
					...
					Biz_08_FolderQuestionFetch.questionFetch(sqlConnection, (JsonObject)(message.body()), resultHandler -> {
        	...
        	logger.info("Event Bus Handler "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_FOLDER_QUESTION_FETCH+" ready to reply.");
