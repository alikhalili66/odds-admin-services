...
import ir.khalili.products.odds.core.biz.transaction.Biz_01_TransactionFetchAll;
import ir.khalili.products.odds.core.constants.AppConstants;
import ir.khalili.products.odds.core.utils.Configuration;

public class VRTCL_01_TransactionFetchAll extends AbstractVerticle {
	private Logger logger = LogManager.getLogger(VRTCL_01_TransactionFetchAll.class);
    ...
    public void start(Promise<Void> startPromise) throws Exception {
    	...
    	try {
        	...
        	vertx.eventBus().consumer(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TRANSACTION_ALL_FETCH, message -> {
        		
        		logger.trace("Event "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TRANSACTION_ALL_FETCH+" recieved with message:"+((JsonObject)(message.body())));
        		...
        		ircJDBC.getConnection(connection -> {
					...
					Biz_01_TransactionFetchAll.fetchAll(vertx, sqlConnection, (JsonObject)(message.body()), resultHandler -> {
        	...
        	logger.info("Event Bus Handler "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TRANSACTION_ALL_FETCH+" ready to reply.");
