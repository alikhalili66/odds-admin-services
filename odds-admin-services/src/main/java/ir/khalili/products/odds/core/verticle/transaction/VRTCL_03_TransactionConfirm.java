...
import ir.khalili.products.odds.core.biz.transaction.Biz_03_TransactionConfirm;
import ir.khalili.products.odds.core.constants.AppConstants;
import ir.khalili.products.odds.core.utils.Configuration;

public class VRTCL_03_TransactionConfirm extends AbstractVerticle {
	private Logger logger = LogManager.getLogger(VRTCL_03_TransactionConfirm.class);
    ...
    public void start(Promise<Void> startPromise) throws Exception {
    	...
    	try {
        	...
        	vertx.eventBus().consumer(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TRANSACTION_CONFIRM, message -> {
        		
        		logger.trace("Event "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TRANSACTION_CONFIRM+" recieved with message:"+((JsonObject)(message.body())));
        		...
        		ircJDBC.getConnection(connection -> {
					...
					Biz_03_TransactionConfirm.confirm(vertx, sqlConnection, (JsonObject)(message.body()), resultHandler -> {
        	...
        	logger.info("Event Bus Handler "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_TRANSACTION_CONFIRM+" ready to reply.");
