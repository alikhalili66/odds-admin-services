...
import ir.khalili.products.odds.core.biz.user.Biz_05_UserFetchPointHistory;
import ir.khalili.products.odds.core.constants.AppConstants;
import ir.khalili.products.odds.core.utils.Configuration;

public class VRTCL_05_UserFetchPointHistory extends AbstractVerticle {
	private Logger logger = LogManager.getLogger(VRTCL_05_UserFetchPointHistory.class);
    ...
    public void start(Promise<Void> startPromise) throws Exception {
    	...
    	try {
        	...
        	vertx.eventBus().consumer(AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_USER_HISTORY_FETCH, message -> {
        		
        		logger.trace("Event "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_USER_HISTORY_FETCH+" recieved with message:"+((JsonObject)(message.body())));
        		...
        		ircJDBC.getConnection(connection -> {
					...
					Biz_05_UserFetchPointHistory.fetchHistory(sqlConnection, (JsonObject)(message.body()), resultHandler -> {
        	...
        	logger.info("Event Bus Handler "+AppConstants.EVNT_BUS_ADR_SRVCS_ODDS_USER_HISTORY_FETCH+" ready to reply.");
